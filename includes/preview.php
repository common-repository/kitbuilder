...
class Preview {
	...
	public function is_preview_mode() {
		...
		if ( ! isset( $_GET['kitbuilder-preview'] ) ) {
			return false;
		}
	...
	public function builder_wrapper() {
		return '<div id="kitbuilder" class="kitbuilder kitbuilder-edit-mode"></div>';
	}

	public function print_custom_css() {
		$container_width = absint( get_option( 'kitbuilder_container_width' ) );

		if ( empty( $container_width ) ) {
			return;
		}

		?><style>.kitbuilder-section.kitbuilder-section-boxed > .kitbuilder-container{max-width: <?php echo esc_html( $container_width ); ?>px}</style><?php
	}
	...
	public function enqueue_styles() {
		...
		wp_register_style(
			'editor-preview',
			KITBUILDER_ASSETS_URL . 'css/editor-preview' . $direction_suffix . $suffix . '.css',
			[],
			KITBUILDER_VERSION
		);
		...
		do_action( 'kitbuilder/preview/enqueue_styles' );
	}
