...
class Compatibility {
	...
	public static function init() {
		...
		if ( defined( 'NGG_PLUGIN_VERSION' ) ) {
			add_filter( 'kitbuilder/utils/get_edit_link', function( $edit_link ) {
		...
		if ( class_exists( '\Ninja_Forms' ) && class_exists( '\NF_Display_Render' ) ) {
			add_action( 'kitbuilder/preview/enqueue_styles', function() {
				ob_start();
		...
		// Add the description (content) tab for a new product, so it can be edited with Kitbuilder
		add_filter( 'woocommerce_product_tabs', function( $tabs ) {
			if ( ! isset( $tabs['description'] ) && Plugin::$instance->preview->is_preview_mode() ) {
				$post = get_post();
				if ( empty( $post->post_content ) ) {
					$tabs['description'] = [
						'title' => __( 'Description', 'kitbuilder' ),
						'priority' => 10,
	...
	/**
	 * Normalize Kitbuilder post meta on import,
	...
	/**
	 * Normalize Kitbuilder post meta on import with the new WP_importer,
	 * We need the `wp_slash` in order to avoid the unslashing during the `add_post_meta`
	 *
	 * @param array $post_meta
	 *
	...
	public static function on_wxr_importer_pre_process_post_meta( $post_meta ) {
		if ( '_kitbuilder_data' === $post_meta['key'] ) {
