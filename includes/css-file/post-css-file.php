...
class Post_CSS_File extends CSS_File {
	...
	const META_KEY = '_kitbuilder_css';
	...
	public function get_element_unique_selector( Element_Base $element ) {
		return '.kitbuilder-' . $this->post_id . ' .kitbuilder-element' . $element->get_unique_selector();
	}
	...
	protected function update_meta( $meta ) {
		update_post_meta( $this->post_id, '_kitbuilder_css', $meta );
	}
	...
	protected function render_css() {
		...
		do_action( 'kitbuilder/post-css-file/parse', $this );
	}

	public function enqueue() {
		if ( ! Plugin::$instance->db->is_built_with_kitbuilder( $this->post_id ) ) {
			return;
		}
	...
	protected function get_file_handle_id() {
		return 'kitbuilder-post-' . $this->post_id;
	}
			...
			if ( ! empty( $control['style_fields'] ) ) {
				foreach ( $values[ $control['name'] ] as $field_value ) {
					$this->add_element_style_rules(
						...
						array_merge( $replacements, [ '.kitbuilder-repeater-item-' . $field_value['_id'] ] )
					);
				}
			}
	...
	private function render_styles( Element_Base $element ) {
		...
		/**
		 * @deprecated, use `kitbuilder/element/parse_css`
		 */
		Utils::do_action_deprecated( 'kitbuilder/element_css/parse_css',[ $this, $element ], '1.0.10', 'kitbuilder/element/parse_css' );

		do_action( 'kitbuilder/element/parse_css', $this, $element );
	}
	...
	private function add_page_settings_rules() {
		...
		$this->add_element_style_rules(
			...
			[ '{{WRAPPER}}' ],
			[ 'body.kitbuilder-page-' . $this->post_id ]
		);
	}
}
