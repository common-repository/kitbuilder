...
class DB {
	...
	public function save_editor( $post_id, $posted, $status = self::STATUS_PUBLISH ) {
		...
		if ( self::STATUS_PUBLISH === $status ) {
			...
			$is_meta_updated = update_metadata( 'post', $post_id, '_kitbuilder_data', $json_value );
			...
			$autosave_id = wp_create_post_autosave( [
				'post_ID' => $post_id,
				'post_title' => __( 'Auto Save', 'kitbuilder' ) . ' ' . date( 'Y-m-d H:i' ),
				'post_modified' => current_time( 'mysql' ),
			] );

			if ( $autosave_id ) {
				update_metadata( 'post',  $autosave_id, '_kitbuilder_data', $json_value );
			}
		}

		update_post_meta( $post_id, '_kitbuilder_version', self::DB_VERSION );
		...
		do_action( 'kitbuilder/editor/after_save', $post_id, $editor_data );
	}
	...
	public function get_plain_editor( $post_id, $status = self::STATUS_PUBLISH ) {
		$data = $this->_get_json_meta( $post_id, '_kitbuilder_data' );

		if ( self::STATUS_DRAFT === $status ) {
			$draft_data = $this->_get_json_meta( $post_id, '_kitbuilder_draft_data' );
	...
	public function remove_draft( $post_id ) {
		delete_post_meta( $post_id, '_kitbuilder_draft_data' );
	}
	...
	public function get_edit_mode( $post_id ) {
		return get_post_meta( $post_id, '_kitbuilder_edit_mode', true );
	}
	...
	public function set_edit_mode( $post_id, $mode = 'builder' ) {
		if ( 'builder' === $mode ) {
			update_post_meta( $post_id, '_kitbuilder_edit_mode', $mode );
		} else {
			delete_post_meta( $post_id, '_kitbuilder_edit_mode' );
		}
	}
	...
	public function copy_kitbuilder_meta( $from_post_id, $to_post_id ) {
		if ( ! $this->is_built_with_kitbuilder( $from_post_id ) ) {
			return;
		}
		...
		foreach ( $from_post_meta as $meta_key => $values ) {
			// Copy only meta with the `_kitbuilder` prefix
			if ( 0 === strpos( $meta_key, '_kitbuilder' ) ) {
				$value = $values[0];

				// The kitbuilder JSON needs slashes before saving
				if ( '_kitbuilder_data' === $meta_key ) {
	...
	public function is_built_with_kitbuilder( $post_id ) {
		$data = $this->get_plain_editor( $post_id );
	 ...
	 * @deprecated 1.4.0
	 */
	public function has_kitbuilder_in_post( $post_id ) {
		return $this->is_built_with_kitbuilder( $post_id );
	}
}
