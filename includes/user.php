...
class User {
	...
	const ADMIN_NOTICES_KEY = 'kitbuilder_admin_notices';
	const INTRODUCTION_KEY = 'kitbuilder_introduction';

	public static function init() {
		add_action( 'wp_ajax_kitbuilder_introduction_viewed', [ __CLASS__, 'set_introduction_viewed' ] );
		add_action( 'wp_ajax_kitbuilder_set_admin_notice_viewed', [ __CLASS__, 'ajax_set_admin_notice_viewed' ] );
	}
	...
	public static function is_current_user_can_edit( $post_id = 0 ) {
		...
		$exclude_roles = get_option( 'kitbuilder_exclude_user_roles', [] );
	...
	public static function ajax_set_admin_notice_viewed() {
		...
		$notices[ (int) $_POST['notice_id'] ] = 'true';
	...
	private static function get_current_introduction() {
		return [
			'active' => true,
			'title' => '<div id="kitbuilder-introduction-title">' .
			           __( 'Two Minute Tour Of Kitbuilder', 'kitbuilder' ) .
			           '</div><div id="kitbuilder-introduction-subtitle">' .
			           __( 'Watch this quick tour that gives you a basic understanding of how to use Kitbuilder.', 'kitbuilder' ) .
			           '</div>',
			'content' => '<div class="kitbuilder-video-wrapper"><iframe src="https://www.youtube.com/embed/6u45V2q1s4k?autoplay=1&rel=0&showinfo=0" frameborder="0" allowfullscreen></iframe></div>',
			'delay' => 2500,
			'version' => 1,
		];
	}
