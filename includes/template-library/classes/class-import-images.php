<?php
namespace Kitbuilder\TemplateLibrary\Classes;
...
class Import_Images {
		...
		$post_id = $wpdb->get_var(
			$wpdb->prepare(
				'SELECT `post_id` FROM %1$s
					WHERE `meta_key` = \'_kitbuilder_source_image_hash\'
						AND `meta_value` = \'%2$s\'
				;',
				$wpdb->postmeta,
	...
	public function import( $attachment ) {
		...
		if ( $info ) {
			...
			return $attachment;
			//return new \WP_Error( 'attachment_processing_error', __( 'Invalid file type', 'kitbuilder' ) );
		}
		...
		update_post_meta( $post_id, '_kitbuilder_source_image_hash', $this->_get_hash_image( $attachment['url'] ) );
