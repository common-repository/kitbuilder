<?php
namespace Kitbuilder\TemplateLibrary;
...
class Source_Local extends Source_Base {
	...
	const CPT = 'kitbuilder_library';

	const TAXONOMY_TYPE_SLUG = 'kitbuilder_library_type';

	const TYPE_META_KEY = '_kitbuilder_template_type';
	...
	public function get_title() {
		return __( 'Local', 'kitbuilder' );
	}

	public function register_data() {
		$labels = [
			'name' => _x( 'My Library', 'Template Library', 'kitbuilder' ),
			'singular_name' => _x( 'Template', 'Template Library', 'kitbuilder' ),
			'add_new' => _x( 'Add New', 'Template Library', 'kitbuilder' ),
			'add_new_item' => _x( 'Add New Template', 'Template Library', 'kitbuilder' ),
			'edit_item' => _x( 'Edit Template', 'Template Library', 'kitbuilder' ),
			'new_item' => _x( 'New Template', 'Template Library', 'kitbuilder' ),
			'all_items' => _x( 'All Templates', 'Template Library', 'kitbuilder' ),
			'view_item' => _x( 'View Template', 'Template Library', 'kitbuilder' ),
			'search_items' => _x( 'Search Template', 'Template Library', 'kitbuilder' ),
			'not_found' => _x( 'No Templates found', 'Template Library', 'kitbuilder' ),
			'not_found_in_trash' => _x( 'No Templates found in Trash', 'Template Library', 'kitbuilder' ),
			'parent_item_colon' => '',
			'menu_name' => _x( 'My Library', 'Template Library', 'kitbuilder' ),
		];
		...
		$args = [
			...
			'hierarchical' => false,
			'supports' => [ 'title', 'thumbnail', 'author', 'kitbuilder' ],
		];

		register_post_type(
			self::CPT,
			apply_filters( 'kitbuilder/template_library/sources/local/register_post_type_args', $args )
		);
		...
		$args = [
			...
			'rewrite' => false,
			'public' => false,
			'label' => _x( 'Type', 'Template Library', 'kitbuilder' ),
		];

		register_taxonomy(
			self::TAXONOMY_TYPE_SLUG,
			self::CPT,
			apply_filters( 'kitbuilder/template_library/sources/local/register_taxonomy_args', $args )
		);
	}
	...
	public function save_item( $template_data ) {
		...
		$template_id = wp_insert_post( [
			'post_title' => ! empty( $template_data['title'] ) ? $template_data['title'] : __( '(no title)', 'kitbuilder' ),
			'post_status' => 'publish',
			'post_type' => self::CPT,
		] );
		...
		do_action( 'kitbuilder/template-library/after_save_template', $template_id, $template_data );

		do_action( 'kitbuilder/template-library/after_update_template', $template_id, $template_data );
	...
	public function update_item( $new_data ) {
		...
		do_action( 'kitbuilder/template-library/after_update_template', $new_data['id'], $new_data );
	...
	public function get_item( $item_id ) {
		...
		return apply_filters( 'kitbuilder/template-library/get_template', $data );
	}
	...
	public function export_template( $item_id ) {
		...
		$filename = 'kitbuilder-' . $item_id . '-' . date( 'Y-m-d' ) . '.json';
	...
	public function post_row_actions( $actions, \WP_Post $post ) {
		if ( self::is_base_templates_screen() ) {
			if ( $this->is_template_supports_export( $post->ID ) ) {
				$actions['export-template'] = sprintf( '<a href="%s">%s</a>', $this->_get_export_link( $post->ID ), __( 'Export Template', 'kitbuilder' ) );
			}
	...
	public function admin_import_template_form() {
		...
		?>
		<div id="kitbuilder-hidden-area">
			<a id="kitbuilder-import-template-trigger" class="page-title-action"><?php _e( 'Import Template', 'kitbuilder' ); ?></a>
			<div id="kitbuilder-import-template-area">
				<div id="kitbuilder-import-template-title"><?php _e( 'Choose an Kitbuilder template JSON file, and add it to the list of templates available in your library.', 'kitbuilder' ); ?></div>
				<form id="kitbuilder-import-template-form" method="post" action="<?php echo admin_url( 'admin-ajax.php' ); ?>" enctype="multipart/form-data">
					<input type="hidden" name="action" value="kitbuilder_import_template">
					<fieldset id="kitbuilder-import-template-form-inputs">
						<input type="file" name="file" accept="application/json" required>
						<input type="submit" class="button" value="<?php _e( 'Import Now', 'kitbuilder' ); ?>">
					</fieldset>
				</form>
			</div>
		</div>
	...
	public function is_template_supports_export( $template_id ) {
		return apply_filters( 'kitbuilder/template_library/is_template_supports_export', true, $template_id );
	}

	private function _get_export_link( $item_id ) {
		return add_query_arg(
			[
				'action' => 'kitbuilder_export_template',
				'source' => $this->get_id(),
				'template_id' => $item_id,
			],
