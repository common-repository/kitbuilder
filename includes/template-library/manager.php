...
use Kitbuilder\TemplateLibrary\Classes\Import_Images;
...
class Manager {
	...
	public function on_export_template_error( \WP_Error $error ) {
		_default_wp_die_handler( $error->get_error_message(), 'Kitbuilder Library' );
	}

	private function register_default_sources() {
		include( KITBUILDER_PATH . 'includes/template-library/classes/class-import-images.php' );
		include( KITBUILDER_PATH . 'includes/template-library/sources/base.php' );
		...
		foreach ( $sources as $source_filename ) {
			include( KITBUILDER_PATH . 'includes/template-library/sources/' . $source_filename . '.php' );
	...
	private function init_ajax_calls() {
		...
		foreach ( $allowed_ajax_requests as $ajax_request ) {
			add_action( 'wp_ajax_kitbuilder_' . $ajax_request, function() use ( $ajax_request ) {
