...
abstract class Control_Base_Units extends Control_Base_Multiple {
	...
	protected function print_units_template() {
		...
		<div class="kitbuilder-units-choices">
			<# _.each( data.size_units, function( unit ) { #>
			<input id="kitbuilder-choose-{{ data._cid + data.name + unit }}" type="radio" name="kitbuilder-choose-{{ data.name }}" data-setting="unit" value="{{ unit }}">
			<label class="kitbuilder-units-choices-label" for="kitbuilder-choose-{{ data._cid + data.name + unit }}">{{{ unit }}}</label>
			<# } ); #>
		</div>
		<# } #>
		<?php
