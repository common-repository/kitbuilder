...
class Elements_Manager {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_kitbuilder_save_builder', [ $this, 'ajax_save_builder' ] );
	}
	...
	public function ajax_save_builder() {
		...
		Plugin::$instance->db->save_editor( (int) $_POST['post_id'], $posted, $status );

		$return_data = [];

		$latest_revision = Revisions_Manager::get_revisions( (int) $_POST['post_id'], [
			'posts_per_page' => 1,
		] );

		$all_revision_ids = Revisions_Manager::get_revisions( (int) $_POST['post_id'], [
			'posts_per_page' => -1,
			'fields' => 'ids',
		], false );
	...
	private function _init_elements() {
		...
		do_action( 'kitbuilder/elements/elements_registered' );
	}

	private function init_categories() {
		$this->_categories = [
			'basic' => [
				'title' => __( 'Basic', 'kitbuilder' ),
				'icon' => 'kb kb-font',
			],
			'general-elements' => [
				'title' => __( 'General Elements', 'kitbuilder' ),
				'icon' => 'kb kb-font',
			],
			'wordpress' => [
				'title' => __( 'WordPress Widgets', 'kitbuilder' ),
				'icon' => 'kb kb-wordpress',
			],
		];
	}

	private function require_files() {
		require_once KITBUILDER_PATH . 'includes/base/element-base.php';

		require KITBUILDER_PATH . 'includes/elements/column.php';
		require KITBUILDER_PATH . 'includes/elements/section.php';
		require KITBUILDER_PATH . 'includes/elements/repeater.php';
	}
}
