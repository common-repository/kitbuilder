...
class Revisions_Manager {
	...
	public static function get_revisions( $post_id = 0, $query_args = [], $parse_result = true ) {
		...
		$query_args['meta_key'] = '_kitbuilder_data';
		...
		foreach ( $posts as $revision ) {
			$date = date_i18n( _x( 'M j @ H:i', 'revision date format', 'kitbuilder' ), strtotime( $revision->post_modified ) );
			...
			$revisions[] = [
				...
				'date' => sprintf( __( '%1$s ago (%2$s)', 'kitbuilder' ), $human_time, $date ),
				'type' => $type,
	...
	public static function save_revision( $revision_id ) {
		...
		Plugin::$instance->db->copy_kitbuilder_meta( $parent_id, $revision_id );
	}

	public static function restore_revision( $parent_id, $revision_id ) {
		Plugin::$instance->db->copy_kitbuilder_meta( $revision_id, $parent_id );
	...
	public static function on_revision_data_request() {
		...
		$revision = Plugin::$instance->db->get_plain_editor( (int) $_POST['id'] );
	...
	public static function on_delete_revision_request() {
		...
		if ( empty( $revision ) ) {
			wp_send_json_error( __( 'Invalid Revision', 'kitbuilder' ) );
		}

		$deleted = wp_delete_post_revision( (int) $_POST['id'] );

		if ( $deleted && ! is_wp_error( $deleted ) ) {
			wp_send_json_success();
		} else {
			wp_send_json_error( __( 'Cannot delete this Revision', 'kitbuilder' ) );
		}
	}

	public static function add_revision_support_for_all_post_types() {
		$post_types = get_post_types_by_support( 'kitbuilder' );
	...
	private static function register_actions() {
		...
		if ( Utils::is_ajax() ) {
			add_action( 'wp_ajax_kitbuilder_get_revision_data', [ __CLASS__, 'on_revision_data_request' ] );
			add_action( 'wp_ajax_kitbuilder_delete_revision', [ __CLASS__, 'on_delete_revision_request' ] );
		}
	}
}
