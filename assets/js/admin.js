...
( function( $, window, document ) {
	...
	var KitbuilderAdminApp = {
		...
		cacheElements: function() {
			this.cache = {
				$body: $( 'body' ),
				$switchMode: $( '#kitbuilder-switch-mode' ),
				$goToEditLink: $( '#kitbuilder-go-to-edit-page-link' ),
				$switchModeInput: $( '#kitbuilder-switch-mode-input' ),
				$switchModeButton: $( '#kitbuilder-switch-mode-button' ),
				$kitbuilderLoader: $( '.kitbuilder-loader' ),
				$builderEditor: $( '#kitbuilder-editor' ),
				$importButton: $( '#kitbuilder-import-template-trigger' ),
				$importArea: $( '#kitbuilder-import-template-area' )
			};
		},
		...
		toggleStatus: function() {
			...
			this.cache.$body
			    .toggleClass( 'kitbuilder-editor-active', isBuilderMode )
			    .toggleClass( 'kitbuilder-editor-inactive', ! isBuilderMode );
		},
		...
		bindEvents: function() {
			...
			self.cache.$switchModeButton.on( 'click', function( event ) {
				...
				if ( 'builder' === self.getEditMode() ) {
					...
					if ( ! $wpTitle.val() ) {
						$wpTitle.val( 'Kitbuilder #' + $( '#post_ID' ).val() );
					}
			...
			$( '#kitbuilder-clear-cache-button' ).on( 'click', function( event ) {
				event.preventDefault();
				var $thisButton = $( this );
			...
			$( '#kitbuilder-replace-url-button' ).on( 'click', function( event ) {
				event.preventDefault();
				var $this = $( this ),
					$tr = $this.parents( 'tr' ),
		...
		initTemplatesImport: function() {
			if ( ! this.cache.$body.hasClass( 'post-type-kitbuilder_library' ) ) {
				return;
			}
			...
			$importButton.on( 'click', function() {
				$( '#kitbuilder-import-template-area' ).toggle();
			} );
		},

		initMaintenanceMode: function() {
			var MaintenanceMode = require( 'kitbuilder-admin/maintenance-mode' );
		...
		animateLoader: function() {
			this.cache.$goToEditLink.addClass( 'kitbuilder-animate' );
		}
	};

	$( function() {
		KitbuilderAdminApp.init();
	} );

}( jQuery, window, document ) );

},{"kitbuilder-admin/maintenance-mode":2}],2:[function(require,module,exports){
...
MaintenanceModeModule = ViewModule.extend( {
	getDefaultSettings: function() {
		return {
			selectors: {
				modeSelect: '.kitbuilder_maintenance_mode_mode select',
				maintenanceModeTable: 'table',
				maintenanceModeDescriptions: '.kitbuilder-maintenance-mode-description',
				excludeModeSelect: '.kitbuilder_maintenance_mode_exclude_mode select',
				excludeRolesArea: '.kitbuilder_maintenance_mode_exclude_roles',
				templateSelect: '.kitbuilder_maintenance_mode_template_id select',
				editTemplateButton: '.kitbuilder-edit-template',
				maintenanceModeError: '.kitbuilder-maintenance-mode-error'
			},
			classes: {
				isEnabled: 'kitbuilder-maintenance-mode-is-enabled'
			}
		};
	},
	...
	bindEvents: function() {
		...
		elements.$templateSelect.on( 'change', function() {
			...
			var editUrl = KitbuilderAdminConfig.home_url + '?p=' + templateID + '&kitbuilder';
...
},{"kitbuilder-utils/view-module":4}],3:[function(require,module,exports){
var Module = function() {
	var $ = jQuery,
